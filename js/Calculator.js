  ...
  */
  checkNumber() {
    ...
    for (let i = $firstNumber.length - 1, j = 0; i >= 0; i--, j++) {
      this.firstNumberArray[i] = parseInt($firstNumber[j].querySelector(".active").innerText);
      this.secondNumberArray[i] = parseInt($secondNumber[j].querySelector(".active").innerText);
      this.resultNumberArray[i] = parseInt($resultNumber[j].querySelector(".active").innerText);
    }
